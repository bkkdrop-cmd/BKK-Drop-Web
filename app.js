...
function getHousingSettings() {
  return db.housing?.settings || {
    roomName: "BKK DROP Housing Room",
    roomCount: 1,
    location: "Bangkok",
    description: "One-room housing slot for BKK DROP customers.",
    status: "open"
  };
}
...
function openHousingBookingModal() {
  ...
  showModal("Booking Housing BKK DROP", `
    <div class="housing-modal-hero">
      <strong>${escapeHtml(settings.roomName || "BKK DROP Housing Room")}</strong>
      <p>Lokasi: ${escapeHtml(settings.location || "Bangkok")} • Room available: 1</p>
    </div>
    ...
    <button class="btn btn-accent btn-full" onclick="window.createHousingBooking()">Request Booking Housing</button>
  `);
}
...
async function createHousingBooking() {
  ...
  const purpose = $("housingPurpose").value;
...
function openEditHousingSettingsModal() {
  ...
  showModal("Housing Settings", `
    <div class="grid grid-2">
      ...
      <div class="field"><label>Lokasi</label><input id="housingLocation" value="${escapeHtml(settings.location || "Bangkok")}" /></div>
    ...
    <button class="btn btn-accent btn-full" onclick="window.saveHousingSettings()">Simpan Housing Settings</button>
  `);
}

async function saveHousingSettings() {
  await update(ref(database, "housing/settings"), {
    roomName: $("housingRoomName").value.trim(),
    location: $("housingLocation").value.trim(),
    roomCount: 1,
    description: $("housingDescription").value.trim(),
...
function renderTabs() {
  if (currentView === "admin") {
    $("tabs").innerHTML = `
      <button class="tab ${adminPage === "warehouse" ? "active" : ""}" onclick="window.setAdminPage('warehouse')">Warehouse Page</button>
      <button class="tab ${adminPage === "kirim" ? "active" : ""}" onclick="window.setAdminPage('kirim')">Kirim Barang</button>
      <button class="tab ${adminPage === "housing" ? "active" : ""}" onclick="window.setAdminPage('housing')">Housing</button>
      <button class="tab" onclick="window.openCreateUserModal()">+ Akun Customer</button>
      <button class="tab" onclick="window.openCreateSlotModal()">+ Slot Warehouse</button>
      <button class="tab" onclick="window.openCreateShipmentModal()">+ Batch Kirim</button>
    `;
  } else {
    $("tabs").innerHTML = `
      <button class="tab ${customerPage === "warehouse" ? "active" : ""}" onclick="window.setCustomerPage('warehouse')">Warehouse Page</button>
      <button class="tab ${customerPage === "kirim" ? "active" : ""}" onclick="window.setCustomerPage('kirim')">Kirim Barang</button>
      <button class="tab ${customerPage === "housing" ? "active" : ""}" onclick="window.setCustomerPage('housing')">Housing</button>
...
function openAddItemModal() {
  ...
  showModal("Tambah Barang", `
    ...
    <div class="field">
      <label>Upload Foto Resi</label>
      <div class="resi-upload-field">
        <input id="itemResiPhoto" type="file" accept="image/*" />
        <p class="muted small">Upload foto resi agar admin bisa mencocokkan barang ini dengan penerimaan di warehouse.</p>
      </div>
    </div>
