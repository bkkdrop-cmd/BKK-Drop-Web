...
async function uploadReceiptImage(file, userId, itemId) {
  ...
  const MAX_ORIGINAL_SIZE = 5 * 1024 * 1024;
  if (file.size > MAX_ORIGINAL_SIZE) {
    throw new Error("Ukuran foto resi terlalu besar. Maksimal 5 MB.");
  }
...
async function deleteResiRecord(itemId, resiId) {
  if (!confirm("Hapus foto resi ini?")) return;
...
async function addItemFromForm() {
  ...
  const estimatedWeight = Number($("itemWeight").value || 0);
  ...
  const maxWeight = Number(slot.maxWeightKg ?? slot.maxWeight ?? slot.maxCustomers ?? 0);

  if (maxWeight && usedWeight + estimatedWeight > maxWeight) {
    toast(`Slot melebihi Max Weight. Sisa kapasitas: ${formatWeight(Math.max(0, maxWeight - usedWeight))} kg.`);
    return;
  }
  ...
  await set(itemRef, {
    ...
    category,
    quantity,
    estimatedPrice: estimatedWeight,
    productLink,
    notes,
    status: "waiting_to_arrive",
    images: {},
  ...
  if (resiFile) {
    try {
      ...
      console.error(error);
      toast(error.message || "Barang tersimpan, tetapi upload resi gagal.");
    }
  }

  closeModal();
  toast(resiFile ? "Barang dan foto resi berhasil ditambahkan." : "Barang berhasil ditambahkan.");
}
...
async function createSlotFromForm() {
  ...
  const maxWeightKg = Number($("newSlotMaxWeightKg").value || 0);
  const status = $("newSlotStatus").value;

  if (!title || !maxWeightKg) {
    toast("Judul slot dan Max Weight wajib diisi.");
    return;
  }
  ...
  closeModal();
  toast("Slot berhasil dibuat berdasarkan Max Weight.");
}
  ...
  showModal("Review Foto Barang", `
    ...
    <div class="admin-resi-box">
      <div class="admin-resi-header">
        <div>
          <strong>Foto Resi dari Customer</strong>
          <p class="muted small">Gunakan foto resi ini untuk mencocokkan paket/barang saat diterima di warehouse.</p>
        </div>
        <span class="pill">${resiImages.length} resi</span>
      </div>
        ...
        ${resiImages.length ? resiImages.map(([resiId, resi]) => `
          <div class="admin-photo-card resi-photo-card">
            ...
            <div class="admin-photo-meta">
              <span class="photo-source-badge resi-source">Foto Resi Customer</span>
              <span>${escapeHtml(formatBytes(resi.compressedSize || resi.originalSize || 0))}</span>
              <button class="btn btn-danger" onclick="window.deleteResiRecord('${itemId}', '${resiId}')">Hapus Resi</button>
            </div>
          </div>
        `).join("") : `<div class="empty">Belum ada foto resi untuk item ini.</div>`}
      </div>
    </div>
...
function exportItemsToCSV(items, filename = "bkk-drop-item-list.csv") {
  ...
  const rows = items.map((item) => {
    ...
    return [
      ...
      item.category || "",
      item.quantity || "",
      (item.estimatedWeight ?? item.estimatedPrice) || "",
...
function getSlotUsedWeight(slotId) {
  const approvedUserIds = new Set(
    idToArray(db.bookings)
      .filter((b) => b.slotId === slotId && b.status === "approved")
      .map((b) => b.userId)
  );

  return idToArray(db.items)
    .filter((item) => item.slotId === slotId && approvedUserIds.has(item.userId))
    .reduce((sum, item) => sum + Number(item.estimatedWeight ?? item.estimatedPrice ?? 0), 0);
}
...
function renderStats() {
  ...
  const adminTotalWeight = allItems.reduce((sum, item) => sum + Number(item.estimatedWeight ?? item.estimatedPrice ?? 0), 0);

  const stats = currentView === "admin"
    ? [["Total Customer", idToArray(db.users).filter((u) => u.role === "customer").length], ["Pending Booking", pendingBookings], ["Total Berat", `${formatWeight(adminTotalWeight)} kg`]]
...
function renderCustomerPanel() {
  ...
  const totalWeight = allMyItems.reduce((sum, item) => sum + Number((item.estimatedWeight ?? item.estimatedPrice) || 0), 0);
  ...
  $("customerPanel").innerHTML = `
    <div class="card warehouse-card">
      <div class="warehouse-header">
        <div>
          <h2>Gudang Saya</h2>
          <p class="muted">Inventory ringkas untuk banyak barang. Cari, filter, dan cek resi/foto diterima dengan cepat.</p>
        </div>
...
function renderSlotCard(slot, myBookings = []) {
  ...
  return `
    <div class="slot-card weight-slot-card">
      <div class="card-row">
        <strong>${escapeHtml(slot.title)}</strong>
      ...
      <div class="slot-weight-box">
        <div class="card-row small">
          <strong>Kuota Berat Warehouse</strong>
          <span>${formatWeight(usedWeight)} / ${formatWeight(maxWeight)} kg</span>
        </div>
        <div class="progress"><span style="width:${percent}%"></span></div>
        <div class="slot-weight-note">${percent}% kapasitas berat terpakai</div>
      </div>
    ...
    <div class="inventory-row">
      ...
      <div class="inventory-main">
        ...
        <div class="inventory-meta">
          ...
          <div><span>Estimasi Berat</span><strong>${escapeHtml(formatWeight(item.estimatedWeight ?? item.estimatedPrice))} kg</strong></div>
        ...
        ${resiImages.length ? `
          <div class="resi-photo-section">
            <div class="received-photo-title">Foto Resi</div>
...
function renderAdminPanel() {
  ...
  $("adminPanel").innerHTML = `
    ...
    <div class="card">
      ...
      <div class="table-wrap" style="margin-top: 14px;">
        <table>
          <thead><tr><th>Customer</th><th>Barang</th><th>Qty</th><th>Status</th><th>Resi</th><th>Foto</th><th>Verifikasi</th><th>Update</th></tr></thead>
          <tbody>
            ${items.map((item) => `
              <tr>
                ...
                <td>
                  <div class="admin-photo-cell">
                    ${Object.values(item.resiImages || {})[0]?.url ? `<img src="${escapeHtml(Object.values(item.resiImages || {})[0].url)}" alt="resi" />` : `<span class="no-photo-dot">No resi</span>`}
                    <span>${Object.keys(item.resiImages || {}).length} resi</span>
                  </div>
                </td>
                <td>
                  <div class="admin-photo-cell">
...
function openAddItemModal() {
  ...
  showModal("Tambah Barang", `
    <div class="grid grid-2">
      ...
      <div class="field"><label>Estimasi Berat (kg)</label><input id="itemWeight" type="number" min="0" step="0.01" value="0" placeholder="Contoh: 0.35" /><small class="field-hint">Gunakan kilogram. Contoh: 0.35 untuk 350 gram.</small></div>
      <div class="field"><label>Link Produk</label><input id="itemLink" placeholder="https://..." /></div>
    </div>
    <div class="field">
      <label>Upload Foto Resi / Bukti Pembelian</label>
      <div class="resi-upload-field">
        <input id="itemResiPhoto" type="file" accept="image/*" />
        <p class="muted small">Upload foto resi atau bukti pembelian agar admin mudah mencocokkan barang saat masuk warehouse.</p>
      </div>
    </div>
...
function openCreateSlotModal() {
  showModal("Buat Slot Baru", `
    <div class="slot-create-intro">
      <strong>Slot berdasarkan Max Weight</strong>
      <p class="muted small">Kapasitas slot akan dihitung dari total estimasi berat barang customer, bukan jumlah customer.</p>
    </div>

    <div class="field"><label>Judul Slot</label><input id="newSlotTitle" placeholder="Warehouse Slot June 2026" /></div>
    <div class="field"><label>Deskripsi</label><textarea id="newSlotDesc" placeholder="Detail slot, periode pengiriman, atau catatan warehouse..."></textarea></div>

    <div class="grid grid-2">
      <div class="field">
        <label>Max Weight (kg)</label>
        <input id="newSlotMaxWeightKg" type="number" min="0" step="0.01" value="20" placeholder="Contoh: 20" />
        <small class="field-hint">Contoh: isi 20 untuk kapasitas maksimal 20 kg.</small>
      </div>
      <div class="field">
        <label>Status</label>
        <select id="newSlotStatus">
