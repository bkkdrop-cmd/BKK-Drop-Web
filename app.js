...
/* CLOUDINARY SYNTAX FIX V50 */
window.addEventListener("error", (event) => {
  console.error("BKK DROP app error:", event.error || event.message);
  const el = document.getElementById("connectionStatus") || document.getElementById("firebaseStatus");
  if (el) {
    el.className = "notice notice-error";
    el.textContent = `App error: ${event.message || "Unknown error"}`;
  }
});


/* SYNTAX FIREBASE FIX V30 */
window.addEventListener("error", (event) => {
  console.error("BKK DROP app error:", event.error || event.message);
...
window.addEventListener("unhandledrejection", (event) => {
  console.error("BKK DROP promise error:", event.reason);
  const el = document.getElementById("connectionStatus");
  if (el) {
    el.className = "notice notice-error";
    el.textContent = `Firebase/App error: ${event.reason?.message || event.reason || "Unknown error"}`;
  }
});
...
/* =========================================================
   CLOUDINARY UPLOAD V47
   Browser upload with unsigned upload preset.
   Stores only returned URL/path metadata in Firebase Database.
   ========================================================= */

function getCloudinaryConfig() {
  // Hard fallback to prevent upload from failing if config.js import/window object is unavailable.
  const hardFallback = {
    cloudName: "djtklfhbs",
  ...
  let importedCfg = {};
  try {
    importedCfg = typeof cloudinaryConfig !== "undefined" ? cloudinaryConfig : {};
  } catch (error) {
    importedCfg = {};
  }

  const windowCfg = window.cloudinaryConfig || {};
  const cfg = { ...hardFallback, ...importedCfg, ...windowCfg };

  return {
    cloudName: cfg.cloudName || "djtklfhbs",
    unsignedUploadPreset: cfg.unsignedUploadPreset || cfg.uploadPreset || "ml_default"
  };
}

function isCloudinaryConfigured() {
  const cfg = getCloudinaryConfig();
  return Boolean(cfg.cloudName && cfg.unsignedUploadPreset);
}
...
async function uploadImageToCloudinary(file, userId = "unknown", folder = "bkk-drop") {
  if (!isCloudinaryConfigured()) {
    throw new Error(`Cloudinary config error. Current cloudName=${getCloudinaryConfig().cloudName}, preset=${getCloudinaryConfig().unsignedUploadPreset}`);
  }
  ...
  formData.append("upload_preset", cfg.unsignedUploadPreset);
  formData.append("folder", `bkk-drop/${folder}/${userId}`);
  ...
  return {
    ...
    folder: data.folder || `bkk-drop/${folder}/${userId}`,
    format: data.format,
    width: data.width,
    height: data.height,
...
async function uploadReceiptImage(file, userId, itemId) {
  return uploadImageToCloudinary(file, userId, `item-photos/${itemId}`);
}

async function uploadAdminReceivedImage(file, userId, itemId) {
  return uploadImageToCloudinary(file, userId, `received-photos/${itemId}`);
}

async function uploadPaymentProofImage(file, userId, invoiceId) {
  return uploadImageToCloudinary(file, userId, `payment-proofs/${invoiceId}`);
}



/* CLOUDINARY UPLOAD FIX V51 */
console.log("BKK DROP Cloudinary config loaded:", getCloudinaryConfig());


/* CLOUDINARY HARDFIX V52 */
console.log("Cloudinary hardfix active:", getCloudinaryConfig());
...
function renderAdminBillingPanel() {
  ...
  const outstandingByCurrency = invoices
    ...
    .reduce((acc, invoice) => {
      ...
      acc[currency] = (acc[currency] || 0) + calculateInvoiceTotal(invoice);
      return acc;
    }, {});

  const outstandingTHB = outstandingByCurrency.THB || 0;
  const outstandingIDR = outstandingByCurrency.IDR || 0;
  ...
  $("adminPanel").innerHTML = `
    <div class="card billing-hero">
      ...
      <div class="billing-summary billing-summary-split">
        <div><span>Total Invoice</span><strong>${invoices.length}</strong></div>
        <div><span>Paid</span><strong>${paidCount}</strong></div>
        <div><span>Outstanding THB</span><strong>${formatCurrency(outstandingTHB, "THB")}</strong></div>
        <div><span>Outstanding IDR</span><strong>${formatCurrency(outstandingIDR, "IDR")}</strong></div>
      </div>
    </div>
...
async function start() {
  try {
    ...
    console.error(error);
    setConnectionStatus("error", "Firebase init failed: " + (error.message || "Check config.js and Realtime Database."));
  }
}
