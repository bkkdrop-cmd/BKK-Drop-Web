...
function showFirebaseStatus(message, type = "warning") {
...
window.addEventListener("error", (event) => {
  ...
  showFirebaseStatus(`App error: ${event.message || "Unknown error"}`, "error");
});

window.addEventListener("unhandledrejection", (event) => {
  console.error("Promise error:", event.reason);
  showFirebaseStatus(`Firebase/App error: ${event.reason?.message || event.reason || "Unknown error"}`, "error");
});
...
async function startFirebaseConnectionV29() {
  const loginBtn = document.getElementById("loginBtn");
  if (loginBtn) {
  ...
  showFirebaseStatus("Mengecek koneksi Firebase...", "warning");

  try {
    await get(ref(database, "users"));
    showFirebaseStatus("Firebase connected. Loading data...", "success");
  } catch (error) {
    console.error("Firebase connection test failed:", error);
    showFirebaseStatus(`Firebase error: ${error.message}. Check Realtime Database rules and databaseURL.`, "error");
    return;
  }
  ...
  try {
    onValue(ref(database), (snapshot) => {
      ...
      db = {
        ...
        shipmentItems: value.shipmentItems || {},
        housing: value.housing || {},
        housingBookings: value.housingBookings || {},
        invoices: value.invoices || {}
      };

      const userCount = Object.keys(db.users || {}).length;
      showFirebaseStatus(`Firebase connected. Users loaded: ${userCount}`, "success");

      if (!userCount && appOptions?.seedDemoData) {
        seedDemoData();
      }
      ...
      showFirebaseStatus(`Firebase read error: ${error.message}`, "error");
    });
  } catch (error) {
    console.error("Firebase listener failed:", error);
    showFirebaseStatus(`Firebase listener error: ${error.message}`, "error");
  }
}



// Start stable Firebase loader.
try {
  startFirebaseConnectionV29();
} catch (error) {
  console.error("Failed to start Firebase V29 loader:", error);
  showFirebaseStatus(`Startup error: ${error.message}`, "error");
}
