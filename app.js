...
function renderCustomerKirimBarangPanel() {
  ...
  $("customerPanel").innerHTML = `
    <div class="card">
      <h2>Kirim Barang</h2>
      <p class="muted">Daftar barang kamu yang sudah dimasukkan admin ke batch pengiriman menuju Indonesia.</p>
      <div class="shipment-items" style="margin-top: 14px;">
        ${myShipmentItems.length ? myShipmentItems.map((item) => {
          ...
          `;
        }).join("") : `<div class="empty">Belum ada barang kamu yang masuk ke batch Kirim Barang.</div>`}
      </div>
    </div>
  `;
}
