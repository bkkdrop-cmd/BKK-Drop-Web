...
function openCreateShipmentModal() {
  showModal("Buat Batch Kirim Barang", `
    ...
    <p class="muted small">Destination otomatis: Indonesia. Max weight adalah total berat yang bisa masuk ke batch penerbangan ini.</p>
    ...
    <div class="grid grid-3">
      ...
      <div class="field">
        ...
        <select id="editShipmentStatus">
          ${statuses.map(([value, label]) => `<option value="${value}" ${shipment.status === value ? "selected" : ""}>${label}</option>`).join("")}
        </select>
      </div>
    </div>
...
function openAddShipmentItemModal(shipmentId) {
  ...
  showModal("Tambah Barang ke Pengiriman", `
...
function exportShipmentCSV(shipmentId) {
  ...
  const rows = items.map((item) => [
    shipment.title || "", shipment.flightDate || "", "Indonesia", item.source || "", item.ownerName || "", item.itemName || "", item.quantity || "", item.weightKg || "", item.notes || ""
  ]);
        ...
        return `
          <div class="card shipment-card">
            ...
            <div class="shipment-capacity">
              <div class="card-row small"><strong>Kapasitas Terbang</strong><span>${formatWeight(usedWeight)} / ${formatWeight(maxWeight)} kg</span></div>
              <div class="progress"><span style="width:${percent}%"></span></div>
            </div>
