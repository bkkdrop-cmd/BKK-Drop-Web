...
function exportItemsToCSV(items, filename = "bkk-drop-item-list.csv") {
  const headers = ["Customer", "Slot", "Nama Barang", "Kategori", "Jumlah", "Estimasi Harga", "Status", "Link Produk", "Catatan", "Jumlah Foto"];
...
function renderCustomerPanel() {
  const myItems = idToArray(db.items).filter((item) => item.userId === currentUser.id);
  ...
  $("customerPanel").innerHTML = `
    <div class="card">
      <h2>Gudang Saya</h2>
      <p class="muted">Daftar barang yang kamu masukkan ke sistem BKK DROP.</p>
      <div class="grid" style="margin-top: 14px;">
        ${myItems.length ? myItems.map(renderItemCard).join("") : `<div class="empty">Belum ada barang. Klik “Tambah Barang” untuk mulai.</div>`}
      </div>
    </div>
    ...
    <div class="item-card">
      <div class="card-row">
        <div>
          ...
          <div class="muted small">${escapeHtml(db.slots[item.slotId]?.title || "No slot")}</div>
        </div>
        <span class="pill">${escapeHtml(statusLabels[item.status] || item.status)}</span>
      </div>

      <div class="grid grid-3">
        <div><div class="stat-label">Kategori</div><div>${escapeHtml(item.category || "-")}</div></div>
        <div><div class="stat-label">Jumlah</div><div>${escapeHtml(item.quantity || 0)}</div></div>
        <div><div class="stat-label">Estimasi</div><div>${escapeHtml(item.estimatedPrice || 0)} THB</div></div>
      </div>

      ${item.notes ? `<div class="muted">${escapeHtml(item.notes)}</div>` : ""}
      ${item.productLink ? `<a class="muted" href="${escapeHtml(item.productLink)}" target="_blank">Buka link produk</a>` : ""}
      ...
      <div class="card-row">
        ...
        <button class="btn btn-danger" onclick="window.deleteItem('${item.id}')">Hapus Barang</button>
      </div>
    </div>
  `;
}
...
function renderAdminPanel() {
  ...
  $("adminPanel").innerHTML = `
    ...
    <div class="card">
      ...
      <div class="table-wrap" style="margin-top: 14px;">
        <table>
          <thead><tr><th>Customer</th><th>Barang</th><th>Qty</th><th>Status</th><th>Foto</th><th>Update</th></tr></thead>
          <tbody>
            ${items.map((item) => `
              <tr>
                ...
                <td>${Object.keys(item.images || {}).length}</td>
                <td>
            ...
            `).join("") || `<tr><td colspan="6">Belum ada barang.</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>
...
function openAddItemModal() {
  ...
  showModal("Tambah Barang", `
    <div class="grid grid-2">
      ...
      <div class="field"><label>Estimasi Harga (THB)</label><input id="itemPrice" type="number" min="0" value="0" /></div>
